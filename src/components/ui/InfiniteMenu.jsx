...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        const grad = ctx.createRadialGradient(x + cellSize/2, y + cellSize/2, cellSize/3, x + cellSize/2, y + cellSize/2, cellSize/2);
  ...
  #onControlUpdate(deltaTime) {
    ...
    this.camera.position[2] += (cameraTargetZ - this.camera.position[2]) / (5 / timeScale);
    this.#updateCameraMatrix();
  }
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div className="infinite-menu-wrapper">
      ...
      {activeItem && (
        <div className="infinite-menu-overlay pointer-events-none">
      ...
      {/* Scroll Hint */}
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-white/20 text-[10px] font-bold uppercase tracking-[0.2em] animate-pulse">
        Drag to Explore
      </div>
    </div>
  );
